...
describe("mix", () => {
	...
	test("removes protocol part from config.hot_url", async () => {
		...
		expect(mix("/unversioned.css")).toBe(
			"//laravel.com/docs/unversioned.css",
		);
	...
	test("prepends env.MIX_ASSET_URL", async () => {
		...
		expect(mix("/unversioned.css")).toBe(
			"https://www.test.com/versioned.css",
		);
	});
	...
	test("env.MIX_HOT_PROXY_URL takes precedence over config.hot_url", async () => {
		...
		expect(mix("/unversioned.css")).toBe(
			"https://www.test.com/unversioned.css",
		);
	});
	...
	test("throws when the public url doesn't exist in the manifest", async () => {
		...
		expect(() => mix("/unversioned.css")).toThrow(
			"Unable to locate Mix file: '/unversioned.css'.",
		);
		...
		expect(() => mix("")).toThrow("Unable to locate Mix file: ''.");

		expect(() => mix("/blabla.js")).toThrow(
			"Unable to locate Mix file: '/blabla.js'.",
		);
	});
});
